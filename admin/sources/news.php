...
function saveMan()
{
	...
	if ($id && $act != 'save_copy') {
		...
		if ($d->update('news', $data)) {
			...
			{
					$fileUpdate = array();
					$file_namea = $func->uploadName($_FILES["file_attach"]["name"]);
					if($file_attach = $func->uploadImage("file_attach", $config['news'][$type]['file_type'], UPLOAD_FILE, $file_namea))
					{
						$row = $d->rawQueryOne("select id, file_attach from #_news where id = ? limit 0,1",array($id));
